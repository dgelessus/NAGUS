...
* (client -> server) **Connect** = 0
  ...
  * **Value of y:** Variable-length integer (maximum 64 bytes).
    Length is implied by the packet byte count.
    Usually,
    *y* is exactly 64 bytes long,
    but it can be shorter if at least 8 of its topmost bits are 0
    (this happens randomly depending on the *b* value generated by the client).
    As a special case,
    when a H'uru client :ref:`requests no encryption <disabling_connection_encryption>`,
    it sends a 0-byte "value" for *y*.
