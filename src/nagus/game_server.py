	...
	async def handle(self, connection: "GameConnection") -> None:
		...
		if self.lock_request:
			...
			try:
				...
				reply_code = pl_messages.ServerReplyMessage.Type.affirm
			else:
				...
				reply_code = pl_messages.ServerReplyMessage.Type.deny
			
			server_reply_message = pl_messages.ServerReplyMessage()
			server_reply_message.receivers.append(self.uoid)
			server_reply_message.type = reply_code
			
			with io.BytesIO() as message_stream:
				pl_messages.Message.creatable_to_stream(server_reply_message, message_stream)
	...
	def inspect_wrapped_message(self, connection: "GameConnection") -> None:
		try:
			...
			if isinstance(self, NetMessageLoadClone):
				...
				if isinstance(message, pl_messages.LoadCloneMessage):
					if isinstance(message, pl_messages.LoadAvatarMessage):
			...
			elif isinstance(message, pl_messages.LoadCloneMessage):
			...
			unexpected_flags = message.flags & ~pl_messages.MessageFlags.all_expected
			if unexpected_flags:
			...
			if pl_messages.MessageFlags.net_propagate not in message.flags:
			...
			if pl_messages.MessageFlags.net_sent in message.flags and not (pl_messages.MessageFlags.net_force in message.flags or pl_messages.MessageFlags.ccr_send_to_all_players in message.flags):
			...
			if (pl_messages.MessageFlags.net_use_relevance_regions in message.flags) != (NetMessageFlags.use_relevance_regions in self.flags):
				logger_pl_message.warning("plMessage %s net_use_relevance_regions flag (%r) doesn't match containing network game message's use_relevance_regions flag (%r)", message.class_description, pl_messages.MessageFlags.net_use_relevance_regions in message.flags, NetMessageFlags.use_relevance_regions in self.flags)
			if (pl_messages.MessageFlags.net_allow_inter_age in message.flags) != (NetMessageFlags.inter_age_routing in self.flags):
				logger_pl_message.warning("plMessage %s net_allow_inter_age flag (%r) doesn't match containing network game message's inter_age_routing flag (%r)", message.class_description, pl_messages.MessageFlags.net_allow_inter_age in message.flags, NetMessageFlags.inter_age_routing in self.flags)
			if (pl_messages.MessageFlags.net_send_unreliable in message.flags) == (NetMessageFlags.needs_reliable_send in self.flags):
				logger_pl_message.warning("plMessage %s net_send_unreliable flag (%r) doesn't match containing network game message's needs_reliable_send flag (%r)", message.class_description, pl_messages.MessageFlags.net_send_unreliable in message.flags, NetMessageFlags.needs_reliable_send in self.flags)
			if (pl_messages.MessageFlags.ccr_send_to_all_players in message.flags) != (NetMessageFlags.route_to_all_players in self.flags):
...
class NetMessageLoadClone(NetMessageGameMessage):
	...
	def inspect_player_load_avatar_message(self, connection: "GameConnection") -> None:
		try:
			...
			with io.BytesIO(message_data) as message_stream:
				(class_index,) = structs.stream_unpack(message_stream, structs.CLASS_INDEX)
				if class_index != pl_messages.LoadAvatarMessage.CLASS_INDEX:
				...
				message = pl_messages.LoadAvatarMessage()
				message.class_index = class_index
				message.read(message_stream)
		except (EOFError, ValueError, pl_messages.UnknownClassIndexError) as exc:
